...
- `apps/web` — React + TypeScript client (Chakra UI, TanStack Query, React Router): home dashboard, `/notes/:view`, `/tracking/:habitId`, `/calendar` and `/calendar/:month` (month-at-a-time transaction calendar), `/stats`, `/source/:noteId`, `/colors` (theme picker); can run against the live services or a static demo snapshot (`VITE_DEMO_MODE`).

...
- `apps/notes-api` — `GET /health`, `GET /notes` (recursively parses every note into a `Note`: frontmatter (with every image found in the raw body added as a `frontmatter.images` array), inline `#tag` hashtags collected into a `tags` array, markdown node tree with wikilinks resolved, images rewritten to the image proxy, and inline hashtags replaced with `tag` nodes, extracted dates, file metadata; optional `?view=` filter and `?includeContent=false`), `GET /views` (per-view note counts/IDs for the view picker, without full content).
...
- `apps/transaction-tracker` — `GET /transactions` (`?month=YYYY-MM`, or `?from=&to=`, defaulting to the current month): reads transactions from note frontmatter and returns the occurrences in that window, both `logged` (a note recording one transaction) and `scheduled` (projected from a note's `recurrence`), plus window totals. Scheduled occurrences are generated on demand for whatever window is asked for, so projection is unbounded — there is no stored horizon. Driven by the optional `transactions` config.
...
- `apps/web` — React + TypeScript client (Chakra UI, TanStack Query, React Router): home dashboard, `/notes/:view`, `/tracking/:habitId`, `/calendar` and `/calendar/:month` (month-at-a-time transaction calendar; fixed-size day cells summarising totals and money-out/in counts, with the day's transactions on hover), `/stats`, `/source/:noteId`, `/colors` (theme picker); can run against the live services or a static demo snapshot (`VITE_DEMO_MODE`).
...
- `packages/app-config` — reads and validates `app.config.json`, merges in the `NOTES_ROOT` env var, and caches the result. Exposes `resolveNotesConfig()` returning a `ResolvedNotesConfig` (`notesDirectory`, `obsidianVault`, `dateFormats`, `attachmentsDirectory`, `createdDateProperty`, `timezone`, `habits`, `transactions`, `views`). Config errors are thrown as plain `Error`s with a message describing what's wrong.
- `packages/markdown` — low-level markdown utilities: `FILE_ID_NAMESPACE` (the shared UUIDv5 namespace every note source seeds `createFileID` with, so services scanning the same vault agree on note ids), `parseFrontMatter`, `parseMarkdownBodyDates`/`extractNoteDates`, `resolveDateFromFrontmatterOrTitle`/`resolveOldestDate`, `collectMarkdownFiles`, `buildObsidianUrl`, `extractImagePaths` (raw-text extraction of every image in a note's body), `extractTags` (raw-text extraction of every inline `#tag` hashtag in a note's body). Owns the `Note` type used across the codebase.
...
**Config shape** (`app.config.json` + `NOTES_ROOT` env var — see `CONTRIBUTING.md` for the full field list): the vault path comes exclusively from the required `NOTES_ROOT` env var, not from `app.config.json`. `obsidianVault` only builds `obsidianUrl` deep links. `dateFormats` finds dates in note bodies. `transactions` names the frontmatter properties a transaction note uses (`amount`, `date`, `description`, `category`, `recurrence`, `recurrenceEnd`), its `currency`, and an optional `folder` to restrict the scan to; every field defaults, so the block is optional. `views` are named filters (`filters` groups of dot-path → expected value, ANDed within a group/ORed across groups, with `$exclude`/`$missing`/`$today`/`$onThisDay` support); callers pass `?view=<id>` to filter.

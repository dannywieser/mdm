...
import { transactionsHandler } from "./handlers/transactions/transactions"
...
const bootstrap = async (): Promise<void> => {
  ...
  startServer(app, {
    ...
    serviceName: "transaction-tracker",
  })
}
...
if (require.main === module) {
  bootstrap().catch((error: unknown) => {
    logger.error(
      ...
      "Unable to start transaction-tracker due to configuration error",
    )
    throw error
  })
}

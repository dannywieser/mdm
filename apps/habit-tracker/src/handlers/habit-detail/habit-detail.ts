...
export const habitDetailHandler: RequestHandler = async (request, response) => {
  ...
  try {
    ...
    const scoreEntries = buildScoreEntries(
      getWindowEntries(entries, today, trackingWindowDays),
      today,
    )
    ...
    const allTimeHighScore = history.reduce(
    ...
    const allTimeHighWindowEntries = history.reduce(
      (max, h) => Math.max(max, h.windowEntries),
      0,
    )
    const lowestDaysTrackedPerPeriod =
      mode === "do-less"
        ? calculateLowestDaysTrackedPerPeriod(
            entries,
            today,

...
`transactions.json` is the one snapshot that cannot fully match the live
service. `transaction-tracker` projects recurring transactions into whatever
month is requested, with no end horizon — a static file has to pick a window,
so the snapshot captures every occurrence from 12 months before the
generation date to 24 months after it, and the demo hook narrows that to
whichever month the calendar is showing. Paging outside the window shows an
empty month in the demo; the live app has no such limit. The recurring
schedules are anchored 13 months before the generation date so their
occurrences already cover the months on either side of it.
...
`app.config.demo.json` is the `app.config.json` used for the demo. It defines 10 grouped views covering every view component (`NotesList`, `NotesGallery`, `NotesReview`, `NotesSummaryTable`) and every filter capability (folder and frontmatter equality, OR groups, `$exclude`, `$missing`, `$today`, `$onThisDay`), plus a single `do-less` habit (screen time) and a `transactions` block pointing at the generated `finance/transactions` folder. The journal notes still carry `exercise`/`reading` frontmatter values for a `do-more` habit to be reintroduced later once that mode is tuned for the demo. View and habit `name`s are all lowercase. The "photo wall" gallery sets `dashboardPreview: true`, so the demo home dashboard also exercises the full-width gallery preview card. "On This Day" pairs `$onThisDay` with the `NotesReview` component — the vault generator guarantees a journal entry exists on that calendar day in every earlier year of its 3+ year timeline, so the review flow always has entries to step through.

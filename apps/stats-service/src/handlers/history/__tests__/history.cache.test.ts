...
const makeResponse = (entriesCreated: number): StatsHistoryResponse => [
  { date: "2026-05-01", entriesCreated, entriesModified: 0, foldersTouched: 0 },
]

describe("createStatsHistoryCache", () => {
  test("returns the freshly computed value on the first call", async () => {
    const cache = createStatsHistoryCache(1000)
  ...
  test("returns the cached value without recomputing while still fresh", async () => {
    let currentTime = 0
    const cache = createStatsHistoryCache(1000, () => currentTime)
    const compute = vi.fn().mockResolvedValue(makeResponse(1))
  ...
  test("shares a single in-flight computation across concurrent callers", async () => {
    const cache = createStatsHistoryCache(1000)
    let resolveCompute: (value: StatsHistoryResponse) => void = () => { /* assigned below */ }
    const compute = vi.fn().mockImplementation(
      () => new Promise<StatsHistoryResponse>((resolve) => { resolveCompute = resolve }),
    )

  ...
  return (
    ...
    >
      <VStack align="stretch" gap={4}>
          ...
          <SimpleGrid columns={3} gap={3}>
            {show.totalNotes && (
              <StatRoot size="sm">
                ...
                <StatValueText>{data.totalNotes.toLocaleString()}</StatValueText>
              </StatRoot>
            )}
            {show.totalFolders && (
              <StatRoot size="sm">
                <StatLabel color="app.textMuted">Folders</StatLabel>
                <StatValueText>{data.totalFolders.toLocaleString()}</StatValueText>
              </StatRoot>
            )}
            {show.totalAttachments && (
              <StatRoot size="sm">
                <StatLabel color="app.textMuted">Attachments</StatLabel>
                <StatValueText>{data.totalAttachments.toLocaleString()}</StatValueText>
              </StatRoot>
            )}
          </SimpleGrid>
        )}
        ...
        {show.notesCreated && (
          <Box>
            <Text fontSize="xs" color="app.textMuted" mb={2} textTransform="uppercase" letterSpacing="wide">
              Created
            </Text>
            <SimpleGrid columns={3} gap={3}>
              <StatRoot size="sm">
              ...
              <StatRoot size="sm">
                ...
                <StatValueText>{data.notesCreated.last365Days}</StatValueText>
              </StatRoot>
            </SimpleGrid>
          </Box>
        )}
        ...
        {show.trends && (
          <Box>
            <Text fontSize="xs" color="app.textMuted" mb={1} textTransform="uppercase" letterSpacing="wide">
            ...
            <Text fontSize="sm">
              ...
              <Text as="span" color="app.textMuted">
                {" "}
                ({data.trends.notesLast30Days} vs {data.trends.notesPrevious30Days} notes)
              </Text>
            </Text>
          </Box>
        )}
              ...
              >
                <defs>
                  <linearGradient id="notesGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="var(--chakra-colors-blue-400)" stopOpacity={0.3} />
                    <stop offset="95%" stopColor="var(--chakra-colors-blue-400)" stopOpacity={0} />
                  </linearGradient>
                </defs>
                <XAxis
                  dataKey="date"
          ...
          <Box>
            ...
            <VStack align="stretch" gap={1}>
              {data.folderBreakdown.slice(0, 8).map(({ folder, count }) => (
                <Box key={folder} display="flex" justifyContent="space-between">
                  <Text fontSize="sm" color="app.textMuted" overflow="hidden" textOverflow="ellipsis" whiteSpace="nowrap">
                    {folder}
                  </Text>
                  <Text fontSize="sm" fontWeight="medium" color="app.text" ml={4} flexShrink={0}>
                    {count}
                  </Text>
                </Box>
              ))}

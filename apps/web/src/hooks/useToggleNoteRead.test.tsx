...
describe('useToggleNoteRead', () => {
  it('posts the note read toggle and updates the cached value', async () => {
    ...
    const { result } = renderHook(() => useToggleNoteRead('note-1'), {
  ...
  it('returns an error when the toggle request fails', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: false,
    })

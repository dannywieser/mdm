...
- `GET /notes`
  ...
  - Success response: `200`
    ...
    ```
  - Every note's raw body text is scanned for image references — standard `![alt](path)` markdown, Obsidian `![[path]]` embeds, and a plain `[label](url)` link whose destination looks like an image file by extension (Bear's inline-image-preview format, e.g. `[3h6HmH](https://images.example.com/i/3h6HmH.jpg)`) — and every image found is resolved and added to `frontmatter.images` as an array, in the order they appear in the note — this is always derived from the body scan and isn't configurable. Notes without frontmatter and without any images return `"frontmatter": null`; notes with images but no other frontmatter return `"frontmatter": { "images": [...] }`. In `content`, a plain image-pointing link is rendered as an `image` node (`alt` taken from the link's text) rather than a `link` node, the same as `![alt](url)` syntax; a link to a non-image URL is left as a normal `link` node. `linkedNotes` (parsed notes referenced via `[[wikilink]]` syntax in the body) is only present on notes that link to others; unmatched wikilinks are left in the body as plain text
...
- `notesSource` (optional, `"obsidian"` or `"bear"`, defaults to `"obsidian"`): selects where `GET /notes` and `GET /views` read notes from.
  ...
  - `"bear"`: reads notes from Redis instead — the `notes:bear` hash that `notes-ingest` populates from `bear-sync` pushes (see `apps/bear-sync/README.md` and `apps/notes-ingest/README.md`). `NOTES_ROOT`/`obsidianVault`/`attachmentsDirectory` are unused in this mode. `REDIS_URL` (environment variable, default `redis://localhost:6379`) configures the connection; the service fails to start if Redis is unreachable when `notesSource` is `"bear"`. `frontmatter.images` is populated the same way as the Obsidian path (images found in the body, including external URLs), but only external image URLs actually resolve to something renderable — Bear has no local attachments directory, so a bare local filename in a Bear note's body just resolves to itself rather than a working `/images?path=...` link.
...
- `views` (optional): array of view configs. Each view has:
  ...
  - `dashboardPreview` (optional, defaults to `false`): only meaningful for views whose `component` is `NotesGallery`. When `true`, the web app's home dashboard renders this view as a full-width preview card showing a pile of scattered, overlapping cover photos instead of the compact name/count stat card. Like `notesGalleryFilters`, this is a UI hint passed through as-is; this API does not act on it.

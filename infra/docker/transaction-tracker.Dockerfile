...
COPY . .
RUN turbo prune transaction-tracker --docker
...
RUN npm run build --workspace transaction-tracker
...
WORKDIR /app/apps/transaction-tracker
USER node

EXPOSE 3006

HEALTHCHECK --interval=30s --timeout=5s --start-period=5s --retries=3 \
  CMD wget --no-verbose --tries=1 --spider "http://localhost:${PORT:-3006}/health" || exit 1

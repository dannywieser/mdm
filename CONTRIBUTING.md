...
- Updating to newer images is manual: run `docker compose pull && docker compose up -d --no-build` (or `npm run docker:update`) from the directory with your `docker-compose.yml`.
